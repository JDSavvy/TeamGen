...
final class GenerateTeamsUseCaseTests: XCTestCase {
    ...
    func testGenerateTeams_InsufficientPlayers_ThrowsError() async {
        ...
        do {
            ...
            if case .insufficientPlayers(let required, let available) = error {
    ...
    func testValidateGeneration_InsufficientPlayers_ReturnsInvalid() async throws {
        ...
        if case .invalid(let error) = result,
           case .insufficientPlayers(let required, let available) = error {
...
private class MockTeamGenerationService: TeamGenerationServiceProtocol {
    ...
    func generateTeams(from players: [PlayerEntity], count: Int, mode: TeamGenerationMode) async throws -> [TeamEntity] {
